...
BACKUP_CMD="pg_dump -h ${POSTGRES_HOST} -p ${POSTGRES_PORT} -U ${POSTGRES_USER} -F c -b -f /backup/\${BACKUP_NAME} ${EXTRA_OPTS} ${POSTGRES_DB}"
...
BACKUP_NAME=\$(date +\%Y.\%m.\%d.\%H\%M\%S).dump
...
if pg_restore -h ${POSTGRES_HOST} -p ${POSTGRES_PORT} -d ${POSTGRES_DB} -U ${POSTGRES_USER} -v \$RESTORE_OPTS \$1 ;then
